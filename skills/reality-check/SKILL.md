...
name: reality-check
description: >
  Compares a developer's stated mental model of a codebase area against what the code actually does today, then writes a graded Mermaid comparison report (a "your model" diagram next to a red/amber/green-outlined "reality" diagram). Use this whenever someone wants to understand an unfamiliar part of a codebase, sanity-check whether their understanding of a system is still accurate, onboard onto a new service, or ground a refactor/incident/planning discussion in what's actually true rather than what used to be true. Trigger on requests like "how does X actually work", "am I still right about how this works", "walk me through the architecture/deployment/data flow of X", "what changed in this system since I last looked", or "before we redesign this, let's make sure I understand it" — even if the user never says "mental model" explicitly. Also trigger before architectural brainstorming or planning work on an existing system, since a shared accurate model is worth establishing first.
---
